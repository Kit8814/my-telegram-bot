...
class SeminarBot:
    def __init__(self, application):
        ...
        self.reminder_tasks = {}  # {subject: task} - задачи для напоминаний (НОВОЕ)
    ...
    async def handle_set_time(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Запускаем задачу для напоминания за 5 минут (НОВОЕ)
        ...
        await update.message.reply_text(
            ...
            f"Напоминание будет отправлено за 5 минут до начала."  # НОВОЕ
        )
    ...
    # НОВЫЙ МЕТОД: Напоминание за 5 минут
    ...
    # НОВЫЙ МЕТОД: Уведомление о напоминании
    async def notify_reminder(self, subject):
        ...
        try:
            ...
            # В реальном боте здесь должна быть рассылка по всем чатам/пользователям
    ...
    # НОВЫЙ МЕТОД: Удаление пользователя с темы
    ...
    # НОВЫЙ МЕТОД: Обработка выбора предмета для удаления
    ...
    # НОВЫЙ МЕТОД: Обработка выбора темы для удаления
    ...
    async def cancel(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        context.user_data.pop('selected_subject', None)
        context.user_data.pop('cancel_action', None)
        context.user_data.pop('removal_subject', None)  # НОВОЕ
